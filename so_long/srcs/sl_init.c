...
/*   Updated: 2023/02/21 16:31:34 by yongjale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (row < (vars->row_length))
	{
		...
		while (col < (vars->col_length))
		{
			if (map[row][col] == '1')
				...
				mlx_put_image_to_window(vars->mlx, vars->win, vars->img5, col * 50, row * 50);
			col++;
		}
		row++;
	}
	mlx_put_image_to_window(vars->mlx, vars->win, vars->img2, vars->l_start.x[0] * 50, vars->l_start.y[0] * 50);
}
